...
PBatteryInformationStruct BatteryStatus::GetBatteryInformation()
{
    return m_data;
}

int BatteryStatus::OnCreateWidget(QWidget **ppWidget)
{
    QWidget *pWidget = new QWidget();
    pWidget->setLayout(new QVBoxLayout());
    pWidget->layout()->setContentsMargins(0, 0, 0, 0);

    this->m_pTreeView = new QTreeView();
    this->m_pTreeView->setEditTriggers(QAbstractItemView::NoEditTriggers);
    this->m_pTreeView->header()->setStretchLastSection(true);
    this->m_pTreeView->header()->setDefaultAlignment(Qt::AlignLeft);

    this->PopulateModule();

    pWidget->layout()->addWidget(m_pTreeView);
...
void BatteryStatus::OnStartLoadingModuleDataSlot()
{
    this->Initialize();

    emit OnLoadingModuleDataCompleteSignal();
    emit OnCreateWidgetSignal(COMPUTER_POWER_MANAGEMENT, this);
}

void BatteryStatus::PopulateModule()
{
    QStandardItem *pItem = 0;

    this->m_pTreeModel->setColumnCount(2);
    this->m_pTreeModel->setRowCount(15);
    this->m_pTreeModel->setHorizontalHeaderLabels(QStringList() << "Property name" << "Property value");

    pItem = new QStandardItem("AC-Line status:");
    this->m_pTreeModel->setItem(0, 0, pItem);

    pItem = new QStandardItem(this->m_data->ACLineStatus);
    this->m_pTreeModel->setItem(0, 1, pItem);

    pItem = new QStandardItem("Battery status:");
    this->m_pTreeModel->setItem(1, 0, pItem);

    pItem = new QStandardItem(this->m_data->BatteryStatus);
    this->m_pTreeModel->setItem(1, 1, pItem);

    pItem = new QStandardItem("Life percent:");
    this->m_pTreeModel->setItem(2, 0, pItem);

    pItem = new QStandardItem(this->m_data->LifePercent);
    this->m_pTreeModel->setItem(2, 1, pItem);

    pItem = new QStandardItem("Life time:");
    this->m_pTreeModel->setItem(3, 0, pItem);

    pItem = new QStandardItem(this->m_data->LifeTime);
    this->m_pTreeModel->setItem(3, 1, pItem);

    pItem = new QStandardItem("Full life-time:");
    this->m_pTreeModel->setItem(4, 0, pItem);

    pItem = new QStandardItem(this->m_data->FullLifeTime);
    this->m_pTreeModel->setItem(4, 1, pItem);

    pItem = new QStandardItem("Technology:");
    this->m_pTreeModel->setItem(5, 0, pItem);

    pItem = new QStandardItem(this->m_data->Technology);
    this->m_pTreeModel->setItem(5, 1, pItem);

    pItem = new QStandardItem("Chemistry:");
    this->m_pTreeModel->setItem(6, 0, pItem);

    pItem = new QStandardItem(this->m_data->Chemistry);
    this->m_pTreeModel->setItem(6, 1, pItem);

    pItem = new QStandardItem("Designed capacity:");
    this->m_pTreeModel->setItem(7, 0, pItem);

    pItem = new QStandardItem(this->m_data->DesignedCapacity);
    this->m_pTreeModel->setItem(7, 1, pItem);

    pItem = new QStandardItem("Full charged capacity:");
    this->m_pTreeModel->setItem(8, 0, pItem);

    pItem = new QStandardItem(this->m_data->FullChargedCapacity);
    this->m_pTreeModel->setItem(8, 1, pItem);

    pItem = new QStandardItem("Current capacity:");
    this->m_pTreeModel->setItem(9, 0, pItem);

    pItem = new QStandardItem(this->m_data->CurrentCapacity);
    this->m_pTreeModel->setItem(9, 1, pItem);

    pItem = new QStandardItem("Low Battery Alert 1:");
    this->m_pTreeModel->setItem(10, 0, pItem);

    pItem = new QStandardItem(this->m_data->LowBatteryAlert1);
    this->m_pTreeModel->setItem(10, 1, pItem);

    pItem = new QStandardItem("Low Battery Alert 2:");
    this->m_pTreeModel->setItem(11, 0, pItem);

    pItem = new QStandardItem(this->m_data->LowBatteryAlert2);
    this->m_pTreeModel->setItem(11, 1, pItem);

    pItem = new QStandardItem("Cycle count:");
    this->m_pTreeModel->setItem(12, 0, pItem);

    pItem = new QStandardItem(this->m_data->CycleCount);
    this->m_pTreeModel->setItem(12, 1, pItem);

    pItem = new QStandardItem("Voltage:");
    this->m_pTreeModel->setItem(13, 0, pItem);

    pItem = new QStandardItem(this->m_data->Voltage);
    this->m_pTreeModel->setItem(13, 1, pItem);

    pItem = new QStandardItem("Wear level:");
    this->m_pTreeModel->setItem(14, 0, pItem);

    pItem = new QStandardItem(this->m_data->WearLevel);
    this->m_pTreeModel->setItem(14, 1, pItem);

    emit OnSendModuleDataToGUISignal(COMPUTER_POWER_MANAGEMENT, m_pTreeView, m_pTreeModel);
}
